...
func (consulClient *DiscoveryClient) Register(
	ctx context.Context,
	serviceName,
	instanceId,
	healthCheckUrl,
...
func (consulClient *DiscoveryClient) DiscoverServices(ctx context.Context, discoveryRequest service.DiscoveryRequest) ([]*InstanceInfo, error) {
	req, err := http.NewRequest("GET", "http://"+consulClient.host+":"+strconv.Itoa(consulClient.port)+"/v1/health/service/"+discoveryRequest.ServiceName, nil)
	if err != nil {
		log.Printf("req format err: %s", err)
		return nil, err
	}
